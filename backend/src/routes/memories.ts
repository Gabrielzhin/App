...
// Helper function to transform memory categories from junction table format to flat array
function transformMemory(memory: any) {
  if (!memory) return memory;
  
  return {
    ...memory,
    categories: memory.categories?.map((cat: any) => cat.category) || [],
  };
}

function transformMemories(memories: any[]) {
...
// Optimized select for timeline/list views (no comments, smaller payload)
const memoryTimelineSelect = {
  id: true,
  title: true,
  content: true,
  photos: true,
...
export async function memoryRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return reply.send(transformMemories(memories));
    }
  );
  ...
  fastify.get<{ Params: { userId: string } }>(
    ...
    async (request: FastifyRequest<{ Params: { userId: string } }>, reply: FastifyReply) => {
      ...
      // Get the friend's profile to check visibility settings
      ...
      // - PRIVATE: Only show memories where current user is tagged
      // - FRIENDS_ONLY: Only show if we're friends
      // - PUBLIC: Show according to memory privacy settings
      if (friend.profileVisibility === 'PRIVATE') {
        // For PRIVATE profiles, only show memories where current user is tagged
        const memories = await prisma.memory.findMany({
          where: {
            isDraft: false,
            userId: friendId,
      ...
      if (friend.profileVisibility === 'FRIENDS_ONLY' && !areFriends) {
        // For FRIENDS_ONLY profiles, non-friends can't see anything
        return reply.send([]);
      }
  ...
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      if (memory.groupId) {
        ...
        // PUBLIC memories are always accessible - no check needed
      }

      return reply.send(transformMemory(memory));
    }
  );
  ...
  fastify.post<{ Body: CreateMemoryBody }>(
    ...
    async (request: FastifyRequest<{ Body: CreateMemoryBody }>, reply: FastifyReply) => {
      const { content, photos, audioUrl, privacy, title, memoryDate, taggedUserIds, aboutUserId, groupId, categoryIds, location, latitude, longitude, moods, isDraft } = request.body;

      // Validate that either content, photos, or audioUrl exist
      if ((!content || content.trim().length === 0) && (!photos || photos.length === 0) && !audioUrl) {
        return reply.code(400).send({ error: 'Content, photos, or audio are required' });
      }
      ...
      return reply.code(201).send(transformMemory(memory));
    }
  );
  ...
  fastify.put<{ Params: { id: string }; Body: UpdateMemoryBody }>(
    ...
    async (
      ...
      const { title, content, photos, audioUrl, privacy, memoryDate, taggedUserIds, categoryIds, location, latitude, longitude, moods, isDraft } = request.body;
  ...
  // PUT /api/memories/:id/core - Toggle core memory status
  fastify.put<{ Params: { id: string }; Body: { isCore: boolean; coreReason?: string } }>(
    '/api/memories/:id/core',
    {
      preHandler: [fastify.authenticate, fastify.requireFullMode],
    },
    async (request: FastifyRequest<{ Params: { id: string }; Body: { isCore: boolean; coreReason?: string } }>, reply: FastifyReply) => {
      try {
        const { id } = request.params;
        const { isCore, coreReason } = request.body;
        ...
        if (memory.userId !== request.user.id) {
          return reply.code(403).send({ error: 'Not authorized to modify this memory' });
        }
        ...
        const updatedMemory = await prisma.memory.update({
          ...
          data: {
            isCore,
            coreReason: isCore ? coreReason : null,
          },
        });

        return reply.send({ memory: transformMemory(updatedMemory) });
      } catch (error) {
        console.error('Failed to update core status:', error);
        return reply.code(500).send({ error: 'Failed to update core status' });
      }
    }
  );

  // GET /api/memories/core - Get user's core memories
  fastify.get(
    '/api/memories/core',
    {
      preHandler: [fastify.authenticate],
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return reply.send({ memories: transformMemories(memories) });
      } catch (error) {
        console.error('Failed to fetch core memories:', error);
        return reply.code(500).send({ error: 'Failed to fetch core memories' });
      }
    }
  );
}
